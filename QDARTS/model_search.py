...
class MixedOp(nn.Module):
  ...
  def __init__(self, C, stride, conv_func, allow8bitprec):
    ...
    for primitive in PRIMITIVES:
      op = OPS[primitive](C //self.k, stride, allow8bitprec=allow8bitprec, affine=False)
  ...
  def forward(self, x, weights, act, wt):
    ...
    temp1 = sum(w * op(xtemp) if idx<3 else w * op(xtemp, act[idx-4], wt[idx-4]) for idx, (w, op) in enumerate(zip(weights, self._ops)))
...
class Network(nn.Module):
  ...
  def __init__(self, C, num_classes, layers, criterion, steps=4, multiplier=4, stem_multiplier=3, allow8bitprec=False):
    ...
    self.allow8bitprec = allow8bitprec
  ...
  def new(self):
    model_new = Network(self._C, self._num_classes, self._layers, self._criterion, allow8bitprec=self.allow8bitprec).cuda()
  ...
  def print_gammas(self):
      print('gamma_activ_celln_preprocess0')
      for i in range(self.num_norm_cells):
          for j in range(self.quant_search_num):
              print('cell', str(i), 'prec', str(j), self.gamma_activ_celln_preprocess0[i, j].item())
      print('gamma_weight_celln_preprocess0')
      for i in range(self.num_norm_cells):
          for j in range(self.quant_search_num):
              print('cell', str(i), 'prec', str(j), self.gamma_weight_celln_preprocess0[i, j].item())
      print('gamma_activ_celln_preprocess1')
      for i in range(self.num_norm_cells):
          for j in range(self.quant_search_num):
              print('cell', str(i), 'prec', str(j), self.gamma_activ_celln_preprocess1[i, j].item())
      print('gamma_weight_celln_preprocess1')
      for i in range(self.num_norm_cells):
          for j in range(self.quant_search_num):
              print('cell', str(i), 'prec', str(j), self.gamma_weight_celln_preprocess1[i, j].item())
      print('gamma_activ_celln')
      num_layers = sum(1 for i in range(self._steps) for n in range(2+i))
      num_ops = len(PRIMITIVES)
      for i in range(self.num_norm_cells):
          for j in range(num_layers):
              for k in range(num_ops - 2):
                  for l in range(self.quant_search_num):
                      print('cell', str(i), 'layer', str(j), 'op', str(k), 'prec', str(l), self.gamma_activ_celln[i, j, k, l].item())
      print('gamma_weight_celln')
      for i in range(self.num_norm_cells):
          for j in range(num_layers):
              for k in range(num_ops - 2):
                  for l in range(self.quant_search_num):
                      print('cell', str(i), 'layer', str(j), 'op', str(k), 'prec', str(l), self.gamma_weight_celln[i, j, k, l].item())

      print('gamma_activ_cellr_preprocess0')
      for i in range(self.num_red_cells):
          for j in range(self.quant_search_num):
              print('cell', str(i), 'prec', str(j), self.gamma_activ_cellr_preprocess0[i, j].item())
      print('gamma_weight_cellr_preprocess0')
      for i in range(self.num_red_cells):
          for j in range(self.quant_search_num):
              print('cell', str(i), 'prec', str(j), self.gamma_weight_cellr_preprocess0[i, j].item())
      print('gamma_activ_cellr_preprocess1')
      for i in range(self.num_red_cells):
          for j in range(self.quant_search_num):
              print('cell', str(i), 'prec', str(j), self.gamma_activ_cellr_preprocess1[i, j].item())
      print('gamma_weight_cellr_preprocess1')
      for i in range(self.num_red_cells):
          for j in range(self.quant_search_num):
              print('cell', str(i), 'prec', str(j), self.gamma_weight_cellr_preprocess1[i, j].item())
      print('gamma_activ_cellr')
      num_layers = sum(1 for i in range(self._steps) for n in range(2+i))
      num_ops = len(PRIMITIVES)
      for i in range(self.num_red_cells):
          for j in range(num_layers):
              for k in range(num_ops - 2):
                  for l in range(self.quant_search_num):
                      print('cell', str(i), 'layer', str(j), 'op', str(k), 'prec', str(l), self.gamma_activ_cellr[i, j, k, l].item())
      print('gamma_weight_cellr')
      for i in range(self.num_red_cells):
          for j in range(num_layers):
              for k in range(num_ops - 2):
                  for l in range(self.quant_search_num):
                      print('cell', str(i), 'layer', str(j), 'op', str(k), 'prec', str(l), self.gamma_weight_cellr[i, j, k, l].item())
