...
def main_search(cfg):
    ...
    print('--------------------------') 
    model = Network(args.init_channels, CLASSES, args.layers, args.auxiliary, genotype, QuantActivConv2d, args.weights_path, device)
    ...
    if cfg.resume:
        ...
        checkpoint = torch.load(cfg.resume_path, map_location=device)['state_dict']

        new_state_dict = {key.lstrip('module.'): value for key, value in checkpoint.items()}
    ...
    #if cfg.dist:
        #model.module.drop_path_prob = args.drop_path_prob * 0 / args.epochs
    #else:
    ...
    temp, bitops = register_forward_hooks(model)
    model.eval()
    with torch.no_grad():
        model(input_data)
    bitops = clever_format([bitops])
...
if __name__ == '__main__':
    ...
    #mp.spawn(main_search, nprocs=num_gpus, args=(args,))
