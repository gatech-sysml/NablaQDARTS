...
def _load_arch(arch_path, names_nbits, device):
    """
...
def quantdarts_cfg(args, local_rank, arch_cfg_path, **kwargs):
    wbits, abits = [2, 4], [2, 4]
    ...
    best_arch, worst_arch = _load_arch(arch_cfg_path, name_nbits, device=device)
    archas = [abits[a] for a in best_arch['alpha_activ']]
    archws = [wbits[w] for w in best_arch['alpha_weight']]

    model = DARTS_CIFAR(drop_path_prob = args.drop_path_prob, conv_func = qm.QuantActivConv2d, device=device, C=48, num_classes=1000, layers = 14, auxiliary=True, genotype=genotypes.PCDARTS)

    # Do backward pass so we can identify which layers do not have a gradient and remove them
    model = model.cuda()
    optimizer = torch.optim.SGD(model.parameters(), lr=0.01)
    optimizer.zero_grad()
    out, out_aux  = model(torch.ones((5, 3, 224, 224)).cuda(local_rank))
...
def fixed_quantdarts(args, abit = 4, wbit = 4):
    """
    Uses fixed bit quantization
    args: 
    """
    
    model = DARTS_CIFAR(drop_path_prob = args.drop_path_prob, conv_func = qm.QuantActivConv2d, C=36, num_classes=10, layers = 20, auxiliary=True, genotype=genotypes.DARTS)
