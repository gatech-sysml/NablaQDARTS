...
#def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.arch=='PCDARTS':
        model = DARTS_CIFAR(drop_path_prob = args.drop_path_prob, conv_func = qm.MixActivConv2d, device=device, C=48, num_classes=1000, layers = 14, auxiliary=True, genotype=genotypes.PCDARTS)
        ...
        out, _ = model(torch.ones((5, 3, 224, 224)).cuda())
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_queue, model, criterion, optimizer, arch_optimizer, epoch, args)
