...
def main_worker(local_rank, args):
    ...
    if args.arch=='DARTS':
        model = DARTS_CIFAR(drop_path_prob = args.drop_path_prob, conv_func = qm.MixActivConv2d, device=device, C=36, num_classes=10, layers = 20, auxiliary=True, genotype=genotypes.DARTS)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, arch_optimizer, epoch, args, local_rank)
...
def train(train_loader, model, criterion, optimizer, arch_optimizer, epoch, args, local_rank):
...
def save_checkpoint(state, is_best, epoch, step_epoch, filename='arch_checkpoint_sub_1.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'arch_model_best_sub_1.pth.tar')
