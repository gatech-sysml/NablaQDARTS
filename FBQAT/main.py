...
#def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.arch_cfg != None and len(args.arch_cfg) > 0:
    ...
    if args.arch=='PCDARTS':
        if args.arch_cfg != None:
            ...
            model = quantdarts_cfg(args, args.arch_cfg, device=device)  # load in best quantized weight configuration for training
        elif args.fixed_bit != -1:
            print("Using fixed bit training with bit width", args.fixed_bit)
            model = fixed_quantdarts(args, device, args.fixed_bit, args.fixed_bit)
        else:
