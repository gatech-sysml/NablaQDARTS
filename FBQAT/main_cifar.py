...
from search_cifar import data_transforms_cifar10
...
parser = argparse.ArgumentParser(description='PyTorch CIFAR10 Main')
...
def main_worker(local_rank, args):
    ...
    if args.distributed:
        ...
        #    args.rank = args.gpu #int(os.environ["RANK"])
        if args.multiprocessing_distributed:
        ...
        dist.init_process_group(backend=args.dist_backend, init_method=args.dist_url,
                                world_size=args.world_size, rank=local_rank) #args.rank)
    # create model
    ...
    if args.arch=='DARTS':
        if args.arch_cfg != None:
            ...
            model = quantdarts_cfg(args, args.arch_cfg)  # load in best quantized weight configuration for training
        elif args.fixed_bit != -1:
            print("Using fixed bit training with bit width", args.fixed_bit)
            model = fixed_quantdarts(args, local_rank, args.fixed_bit, args.fixed_bit)
        else:
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            if local_rank is not None:
                ...
                best_acc1 = best_acc1.to(local_rank)
    ...
    if args.distributed:
        train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset, num_replicas=ngpus_per_node, rank=local_rank)
    else:
        train_sampler = None
    ...
    if args.evaluate:
        validate(val_loader, model, criterion, args, local_rank)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        acc1 = validate(val_loader, model, criterion, args, local_rank)
...
def save_checkpoint(state, is_best, epoch, step_epoch, filename='checkpoint_qat_1.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'model_best_qat_1.pth.tar')
